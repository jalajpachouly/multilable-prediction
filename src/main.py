...
def visualize_description_length(train: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig(f'..\\output\\description_length_distribution_{type}.png')  # Save the plot
    plt.close()
...
def visualize_class_distribution(y_train: pd.DataFrame, y_test: pd.DataFrame, save_path='class_distribution.png'):
    ...
    plt.tight_layout()
    # Ensure the parentheses are correctly placed
    plt.savefig(f'..\\output\\{type}_{save_path}')
    plt.close()
...
def W_Cloud(train: pd.DataFrame, token: str, max_words=50):
    ...
    plt.tight_layout()
    plt.savefig(f'..\\output\\wordcloud_{token}.png')  # Save the plot
    plt.close()
...
def visualize_f1_scores(methods: pd.DataFrame):
    ...
    plt.xlabel('Model')
    plt.tight_layout()
    plt.savefig(f'..\\output\\f1_score_distribution.png')  # Save the plot
    plt.close()
...
def visualize_all_metrics_boxplot(methods: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig(f'..\\output\\all_metrics_comparison_boxplot_{type}.png')  # Save the plot
    plt.close()
...
def visualize_nb_metrics(methods: pd.DataFrame):
    ...
    plt.xlabel('Labels')
    plt.tight_layout()
    plt.savefig(f'..\\output\\mnb_metrics_per_label_{type}.png')  # Save the plot
    plt.close()
...
def visualize_correlation_matrix(y_train: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig(f'..\\output\\label_correlation_matrix_{type}.png')  # Save the plot
    plt.close()
...
def visualize_label_frequency(y_train: pd.DataFrame):
    ...
    plt.tight_layout()
    plt.savefig(f'..\\output\\label_frequency_{type}.png')  # Save the plot
    plt.close()
...
def plot_top_features(selected_features, chi2_scores_max, top_k_plot=20):
    ...
    plt.ylabel('Feature')
    plt.tight_layout()
    plt.savefig(f'..\\output\\chi2_features_{type}.png')
    plt.close()
...
def main():
    ...
    # Load Data
    # ----------------------------
    csv_path = "..\\dataset\\data_ml_1.csv"
    try:
