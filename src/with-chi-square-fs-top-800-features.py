...
def prepare_data(X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.DataFrame, top_k=800, vocabulary=None):
    """
    ...
    # After fitting the TF-IDF vectorizer to your training data
    total_features = len(vector.get_feature_names_out())
...
def main(data_type='Unbalanced'):
    ...
    try:
        X_train_tfidf, X_test_tfidf, selected_features, chi2_scores_max, vector = prepare_data(
            X_train_df, X_test_df, y_train_df, top_k=top_k_values,vocabulary=wordcloud_vocab
        )
    except Exception as e:
...
if __name__ == "__main__":
    top_k_values = 1550  # Initialize `top_k_values`
    for i in range(10):  # Loop runs 20 times (0-19)
        print(f"\n-------------------RUN : {i + 1} for K = {top_k_values}---------------------------")
