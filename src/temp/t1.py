...
def load_data(csv_path: str):
    ...
    required_columns = ['additional_info', 'type_blocker', 'type_bug', 'type_documentation',
                        'type_enhancement', 'type_regression', 'type_task']
    ...
    y = data[['type_blocker', 'type_bug', 'type_documentation', 'type_enhancement', 'type_regression', 'type_task']]
...
def prepare_data(X_train: pd.DataFrame, X_test: pd.DataFrame, y_train: pd.DataFrame, top_k=50):
    """
...
def W_Cloud(train: pd.DataFrame, token: str):
    """
...
def visualize_label_frequency(y_train: pd.DataFrame):
    ...
    labels = ["type_blocker", "type_bug", "type_documentation", "type_enhancement", "type_regression", "type_task"]
...
def main():
    ...
    # Load Data
    # ----------------------------
    csv_path = "D:\\Phd-Jalaj\\dataset\\final\\ml-training1.csv"  # Update this path if necessary
    try:
    ...
    # ----------------------------
    # Visualize Word Clouds for Each Label
    # ----------------------------
    labels = ["type_blocker", "type_bug", "type_documentation", "type_enhancement", "type_regression", "type_task"]
    for label in labels:
        W_Cloud(X_train_df.join(y_train_df), label)  # Ensure 'additional_info' and labels are present
    ...
    try:
        X_train, X_test, selected_features, chi2_scores_max, vector = prepare_data(
            X_train_df, X_test_df, y_train_df, top_k=top_k
        )
    except Exception as e:
