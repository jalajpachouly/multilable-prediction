...
def prepare_data(
        ...
        top_k=500,  # Changed from 50 to 500
        vocabulary=None
):
    """
...
def main(data_type='Unbalanced'):
    ...
    # ----------------------------
    # Prepare Data with Vocabulary (With Chi-Square Feature Selection)
    # ----------------------------
    ...
    # ------------------------------------------------------------
    # NEW BLOCK ADDED TO ADDRESS REVIEWER COMMENT:
    # Provide comparison BEFORE and AFTER Chi-Square Feature Selection
    # ------------------------------------------------------------

    print("\n===== Evaluating Performance WITHOUT Feature Selection (Full TF-IDF) =====")
    # Vectorizer with no feature selection at all (entire vocabulary, ignoring Chi-Square)
    vector_no_fs = TfidfVectorizer(
        ngram_range=(1, 1),
        analyzer='word',
        stop_words='english',
        strip_accents='unicode',
    ...
    X_train_tfidf_no_fs = vector_no_fs.fit_transform(X_train_df['report'])
    X_test_tfidf_no_fs = vector_no_fs.transform(X_test_df['report'])

    # Define the same classifiers
    clf1_no_fs = ClassifierChain(MultinomialNB())
    clf2_no_fs = ClassifierChain(LogisticRegression(max_iter=10000))
    clf3_no_fs = ClassifierChain(RandomForestClassifier(n_estimators=100, random_state=42))

    # Cross-validation without feature selection
    meth_cv_no_fs = []
    for clf, model_name in zip([clf1_no_fs, clf2_no_fs, clf3_no_fs],
                               ['MultinomialNB', 'LogisticRegression', 'RandomForest']):
        print(f"\n===== Cross-Validating {model_name} (No FS) =====")
        cv_scores_no_fs = cross_validation_score_multilabel(clf, X_train_tfidf_no_fs, y_train_np)
        meth_cv_no_fs.append(
            {'Model': model_name, 'Recall(NoFS)': cv_scores_no_fs['Recall'], 'F1(NoFS)': cv_scores_no_fs['F1']})

    meth_cv_no_fs = pd.DataFrame(meth_cv_no_fs)

    print("\nCross-validation results (No Feature Selection):")
    print(meth_cv_no_fs)

    # Evaluate on Test Set (No FS)
    results_nb_no_fs = evaluate_classifier(clf1_no_fs, 'MultinomialNB(NoFS)',
                                           X_train_tfidf_no_fs, y_train_np,
                                           X_test_tfidf_no_fs, y_test_np, label_names)
    results_lr_no_fs = evaluate_classifier(clf2_no_fs, 'LogisticRegression(NoFS)',
                                           X_train_tfidf_no_fs, y_train_np,
                                           X_test_tfidf_no_fs, y_test_np, label_names)
    results_rf_no_fs = evaluate_classifier(clf3_no_fs, 'RandomForest(NoFS)',
                                           X_train_tfidf_no_fs, y_train_np,
                                           X_test_tfidf_no_fs, y_test_np, label_names)

    combined_no_fs = results_nb_no_fs + results_lr_no_fs + results_rf_no_fs
    df_results_no_fs = pd.DataFrame(combined_no_fs)

    # ------------------------------------------------------------
    # Now proceed with Feature-Selected pipeline (already prepared)
    # ------------------------------------------------------------

    print("\n===== Evaluating Performance WITH Chi-Square Feature Selection =====")

    # ----------------------------
    # Define Classifiers (With FS)
    # ----------------------------
    ...
    # ----------------------------
    # Cross-Validation for Traditional Models (With FS)
    # ----------------------------
    meth_cv = []
    for clf, model_name in zip([clf1, clf2, clf3], ['MultinomialNB', 'LogisticRegression', 'RandomForest']):
        print(f"\n===== Cross-Validating {model_name} (With FS) =====")
        cv_scores = cross_validation_score_multilabel(clf, X_train_tfidf, y_train_np)
        meth_cv.append({'Model': model_name, 'Recall(FS)': cv_scores['Recall'], 'F1(FS)': cv_scores['F1']})
    meth_cv = pd.DataFrame(meth_cv)

    print("\nCross-validation results (With Feature Selection):")
    print(meth_cv[['Model', 'Recall(FS)', 'F1(FS)']])

    # ----------------------------
    # Evaluate Classifiers on Test Set (With FS)
    # ----------------------------
    ...
    # Merge cross-validation comparisons side by side
    comparison_cv = pd.merge(meth_cv_no_fs, meth_cv, on='Model', how='outer')
    print("\n===== Comparison of Cross-Validation Results (No FS vs. With FS) =====")
    print(comparison_cv)

    # ----------------------------
    # Combine results (With FS)
    # ----------------------------
    combined_results = results_nb + results_lr + results_rf
    df_results_fs = pd.DataFrame(combined_results)

    # ----------------------------
    # Deep Learning Model (With FS)
    # Cross-Validation
    # ----------------------------
    print("\n===== Training and Evaluating Deep Learning Model via Cross-Validation (With FS) =====")
    ...
    print("\nDeep Learning Cross-validation results (With FS):")
    print(f"Recall: {deep_learning_cv_scores['Recall']:.4f}")
    print(f"F1-score: {deep_learning_cv_scores['F1']:.4f}")

    # ----------------------------
    # Train Deep Learning Model on Entire Training Set (With FS)
    # ----------------------------
    print("\n===== Training Deep Learning Model on Entire Training Set (With FS) =====")
    ...
    # ----------------------------
    # Evaluate Deep Learning Model on Test Set (With FS)
    # ----------------------------
    results_dl = evaluate_deep_learning_model(deep_learning_model, X_test_tfidf, y_test_np, 'MLP', label_names)

    # ----------------------------
    # Prepare Data for CNN (No FS vs FS is less straightforward with sequence-based,
    # so standard approach is shown here)
    # ----------------------------
    ...
    # ----------------------------
    # Combine All Final Results (Traditional + DL)
    # ----------------------------
    combined_results_fs_all = results_nb + results_lr + results_rf + results_dl + results_cnn
    df_results_fs_all = pd.DataFrame(combined_results_fs_all)

    # Convert 'Hamming Loss' to numeric
    df_results_fs_all['Hamming Loss'] = pd.to_numeric(df_results_fs_all['Hamming Loss'], errors='coerce')
    ...
    visualize_f1_scores(df_results_fs_all, data_type)

    # Box plot comparing Recall, F1-score, and Hamming Loss across all models
    visualize_all_metrics_boxplot(df_results_fs_all, data_type)

    # Bar plots for Multinomial Naive Bayes metrics (With FS)
    visualize_nb_metrics(df_results_fs_all, data_type)
